...
def build_model(input_shape):
	...
	xout_diam = Dense(1, name='o_diam', activation='relu')(xpool_malig) #relu output
	xout_lob = Dense(1, name='o_lob', activation='relu')(xpool_malig) #relu output
	xout_spic = Dense(1, name='o_spic', activation='relu')(xpool_malig) #relu output
	...
	model.compile(optimizer=opt,loss={'o_diam':'mae', 'o_lob':'mae', 'o_spic':'mae', 'o_mal':'mae'},
								loss_weights={'o_diam':1.0, 'o_lob':5.0, 'o_spic':5.0, 'o_mal':5.0})
...
def train_model_on_stage(stage,model,fast_start=False):
	...
	name = 'model_des_v35_multi_' + str(stage) + '_{epoch:02d}.h5'
	...
	# csv_logger = CSVLogger(filename = 'model_des_v35_multi_' + str(stage) + '_trainlog.csv',append=True)
...
if __name__ == '__main__':
	...
	#v35_multi_describer 64 model: 4.5 train loss and 4.27 valid loss @ 15
	
	#so clearly the v35_multi_describer model isn't as good as the v24. 
	...
	# model_64 = load_model("F:\Flung\descriptor models\model_des_v35_multi_64_08.h5")
	# model_96 = build_model((1,96,96,96))
	 # model_256 = build_model((1,256,256,256))
	
	model_32.summary()
	
	# model_32.load_weights('model_v35_multi_describer_weights_temp.h5')
	...
	model_32.save_weights('model_v35_multi_describer_weights_temp.h5')
	model_64.load_weights('model_v35_multi_describer_weights_temp.h5')
	model_64 = train_model_on_stage(64, model_64,fast_start=False)
	model_64.save_weights('model_v35_multi_describer_weights_temp64.h5')
