...
def dense_branch(xstart, name, outsize=1,activation='sigmoid'):
	...
	xdense_ = Dropout(2.0/32.0)(xdense_)
...
def build_model(input_shape):
	...
	x3_diam = conv_block(x2_merged,36,activation='relu') #outputs 25 + 16 ch = 41
	x3_lob = conv_block(x2_merged,36,activation='relu') #outputs 25 + 16 ch = 41
	x3_spic = conv_block(x2_merged,36,activation='relu') #outputs 25 + 16 ch = 41
	...
	x4_diam = conv_block(x3_diam_merged,48,activation='relu') #outputs 25 + 16 ch = 41
	x4_lob = conv_block(x3_lob_merged,48,activation='relu') #outputs 25 + 16 ch = 41
	x4_spic = conv_block(x3_spic_merged,48,activation='relu') #outputs 25 + 16 ch = 41
	...
	x5_diam = conv_block(x4_diam_merged,64,pool=False) #outputs 25 + 16 ch = 41
	x5_lob = conv_block(x4_lob_merged,64,pool=False) #outputs 25 + 16 ch = 41
	x5_spic = conv_block(x4_spic_merged,64,pool=False) #outputs 25 + 16 ch = 41
	x5_malig = conv_block(x4_malig_merged,64,pool=False) #outputs 25 + 16 ch = 41
	...
	xout_diam = dense_branch(xpool_diam,name='o_diam',outsize=1,activation='relu')
	xout_lob = dense_branch(xpool_lob,name='o_lob',outsize=1,activation='relu')
	xout_spic = dense_branch(xpool_spic,name='o_spic',outsize=1,activation='relu')
	xout_malig = dense_branch(xpool_malig,name='o_mal',outsize=1,activation='relu')
	...
	model.compile(optimizer=opt,loss={'o_diam':'mae', 'o_lob':'mae', 'o_spic':'mae', 
										'o_mal':'mae'},
								loss_weights={'o_diam':1.0, 'o_lob':2.0, 'o_spic':2.0, 'o_mal':5.0})
	return model
