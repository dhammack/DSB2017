...
def build_model(input_shape):
	...
	xpool_malig = BatchNormalization(momentum=0.99)(GlobalMaxPooling3D()(x5_malig))
	...
	xpool_diam = BatchNormalization(momentum=0.99)(GlobalMaxPooling3D()(x5_diam))
	...
	xpool_lob = BatchNormalization(momentum=0.99)(GlobalMaxPooling3D()(x5_lob))
	xout_lob = Dense(1, name='o_lob', activation='softplus')(xpool_lob) #relu output

	x5_spic = conv_block(x4_merged,64) #outputs 25 + 16 ch = 41
	xpool_spic = BatchNormalization(momentum=0.99)(GlobalMaxPooling3D()(x5_spic))
...
def train_model_on_stage(stage,model,fast_start=False,split=True,fine_tune=False):
	...
	name = 'model_des_v37c_' + str(stage) + '_{epoch:02d}.h5'
	...
	if fine_tune:
		...
		name = 'model_des_v37c_' + str(stage) + '_finetune_{epoch:02d}.h5'
		chkp = ModelCheckpoint(filepath=name)
...
if __name__ == '__main__':
	...
	import data_generator_fn2_des as data_generator_fn
	from keras.regularizers import l2
	
	np.random.seed(32512)
	print 'loading data'
	...
	#v37c_mse_describer 64 model: 4.5 train loss and 4.27 valid loss @ 15
	
	#so clearly the v37c_mse_describer model isn't as good as the v24. 
	...
	# model_64 = load_model("F:\Flung\descriptor models\model_des_v37c_mse_64_08.h5")
	# model_96 = build_model((1,96,96,96))
	 # model_256 = build_model((1,256,256,256))
	
	model_32.summary()
	
	# model_32.load_weights('model_v37c_mse_describer_weights_temp.h5')
	# print 'prior model (v34 desc) obtained malig MAE train/val 0.357613328172 0.454452694203'
	print 'prior model (v37 des mse v1) obtained 1.157/1.15 ish trn/val @25'
	...
	model_32.save_weights('model_v37c_mse_describer_weights_temp.h5')
	model_64.load_weights('model_v37c_mse_describer_weights_temp.h5')
	model_64 = train_model_on_stage(64, model_64,fast_start=False,split=True,fine_tune=True)
	model_64.save_weights('model_v37c_mse_describer_weights_temp64.h5')
	print 'this model is nearly the same as v37 except using a different train/val split and slightly different settings'
