...
def conv_block(x_input, num_filters,pool=True,activation='relu',init='orthogonal',double=False):
	...
	#x1 = Activation('relu')(x1)
        #x1 = PReLU(shared_axes=[2,3,4],trainable=False)(x1)
        x1 = LeakyReLU(.4)(x1)
        if double:
...
def build_model(input_shape):
	...
	x1 = conv_block(xin,8,activation='relu', double=True)
	x1_ident = AveragePooling3D()(xin)
	x1_merged = merge([x1, x1_ident],mode='concat', concat_axis=1)
	
	x2_1 = conv_block(x1_merged,24,activation='relu',double=True) #outputs 37 ch
	...
	x3_malig = conv_block(x2_merged,48,activation='relu',double=True) #outputs 25 + 16 ch = 41
...
def train_model_on_stage(stage,model,fast_start=False,split=True,fine_tune=False):
	...
	if split:
		...
		if ppos is None:
                    ppos = np.random.permutation(Xpos.shape[0])
                Xpos = Xpos[ppos] #np.roll(Xpos,axis=0,shift=int(.8*Xpos.shape[0]))
                IXpos = IXpos[ppos] #np.roll(IXpos, axis=0, shift=int(.8*Xpos.shape[0]))
		...
		IXneg = np.load(r"/home/ec2-user/data/IXrandom_temp_v5_" + str(stage) + ".npy")
	        if pneg is None:
                    pneg = np.random.permutation(Xneg.shape[0])
                Xneg = Xneg[pneg] #np.roll(Xneg,axis=0,shift=int(.8*Xneg.shape[0]))
                IXneg = IXneg[pneg] #np.roll(IXneg, axis=0, shift=int(.8*Xneg.shape[0]))
	...
	name = 'model_des_v37g_' + str(stage) + '_{epoch:02d}.h5'
	...
	if fine_tune:
                print 'waiting for data gen to finish before fine tuning'
                while data_gen_process.is_alive():
			time.sleep(1)
	        Xpos = np.load(r"/home/ec2-user/data/Xpositive_temp_v5_" + str(stage) + ".npy")
	        IXpos = np.load(r"/home/ec2-user/data/IXpositive_temp_v5_" + str(stage) + ".npy")
		...
		name = 'model_des_v37g_' + str(stage) + '_finetune_{epoch:02d}.h5'
		chkp = ModelCheckpoint(filepath=name)
...
if __name__ == '__main__':
	...
	# model_64 = load_model("F:\Flung\descriptor models\model_des_v37g_64_08.h5")
	# model_96 = build_model((1,96,96,96))
	 # model_256 = build_model((1,256,256,256))
	
	model_32.summary()
	
	# model_32.load_weights('model_v37g_describer_weights_temp.h5')
	...
	model_32.save_weights('model_v37g_describer_weights_temp.h5')
	model_64.load_weights('model_v37g_describer_weights_temp.h5')
	model_64 = train_model_on_stage(64, model_64,fast_start=False,split=True,fine_tune=True)
	model_64.save_weights('model_v37g_describer_weights_temp64.h5')
