...
def train_model_on_stage(stage,model,fast_start=False,split=True,fine_tune=False):
	...
	if split:
		...
		Xpos = np.roll(Xpos,axis=0,shift=int(.8*Xpos.shape[0]))
                IXpos = np.roll(IXpos, axis=0, shift=int(.8*Xpos.shape[0]))
		...
		IXneg = np.load(r"/home/ec2-user/data/IXrandom_temp_v5_" + str(stage) + ".npy")
	        Xneg = np.roll(Xneg,axis=0,shift=int(.8*Xneg.shape[0]))
                IXneg = np.roll(IXneg, axis=0, shift=int(.8*Xneg.shape[0]))
	...
	name = 'model_des_v37d_' + str(stage) + '_{epoch:02d}.h5'
	...
	for epoch in range(12, nb_epoch):
	...
	if fine_tune:
		...
		name = 'model_des_v37d_' + str(stage) + '_finetune_{epoch:02d}.h5'
		chkp = ModelCheckpoint(filepath=name)
...
if __name__ == '__main__':
	...
	# model_64 = load_model("F:\Flung\descriptor models\model_des_v37d_64_08.h5")
	# model_96 = build_model((1,96,96,96))
	 # model_256 = build_model((1,256,256,256))
	
	model_32.summary()
	
	# model_32.load_weights('model_v37d_describer_weights_temp.h5')
	...
	#model_32.save_weights('model_v37d_describer_weights_temp.h5')
	#model_64.load_weights('model_v37d_describer_weights_temp.h5')
	model_64pt = load_model("model_des_v37d_64_12.h5")
	model_64pt.save_weights('weights_temp.h5')
	model_64 = load_weights('weights_temp.h5',by_name=True)
	model_64 = train_model_on_stage(64, model_64,fast_start=False,split=True,fine_tune=False)
	model_64.save_weights('model_v37d_describer_weights_temp64.h5')
